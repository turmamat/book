<template>
	...
	<view class="application" v-if="loading==true">
		<view class="nonull" v-if="records.length!=0">
			<view v-for="record in records" @click="touUpload(record.bookid,record.bookName)" class="record">
				<view class="left">
...
<script>
	export default{
		...
		methods:{
			touUpload(id,bookName){
				uni.navigateTo({
					url:"/pages/user/uploadChapter/detail/datail?id="+id+"&bookName="+bookName
				})
			}
		}
	}
